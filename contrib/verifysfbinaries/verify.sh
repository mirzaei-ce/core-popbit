...
###   This script attempts to download the signature file SHA256SUMS.asc from popbit.org
...
WORKINGDIR="/tmp/popbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://popbit.org/bin/"
VERSIONPREFIX="popbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'popbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Popbit Core binary release signing key installed?"
   fi
